var Render = {
	...
	render: function(dane){
		...
		if(Akt.dane.zapisano){
			Render.dymek(dane, 900, 20, "ZAPISANO", "white", 14);
		}
	},
		...
		if(mysz.click){
			...
			if(Fizyka.kolizjaD(polecenia, myszObj)){
				...
				if(d == "nowa gra"){
					...
					Silnik.zapis(dane);
				}else if(d == "auto zapis"){
					dane.config.autoZapis = !dane.config.autoZapis;
					alert("auto zapis = "+dane.config.autoZapis);
				}else if(d == "muzyka"){
					Silnik.muzyka(dane);
				}else if(d == "res pos"){
					dane.gracz.x = 495;
					...
					dane.gracz.x += __.rand(-300, 300);
					dane.gracz.y += __.rand(-300, 300);
				}else if(d == "sklep"){
					let skl = prompt(
						"--POSIADAMY--\n"+
						"- hp \t\t --> 10 monet\n"+
			...
			if(Fizyka.kolizjaD(instrokcja, myszObj)){
				...
				alert(
					...
					"- auto zapis \t\t --> co jakisz czas zapisuje postęp\n"+
